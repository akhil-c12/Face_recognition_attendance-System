...
def get_attendance_logs_for_subjects(subject_ids):
    if not subject_ids:
        return []
    response = supabase.table('attendance_logs').select('subject_id,timestamp').in_('subject_id', subject_ids).execute()
    return response.data
